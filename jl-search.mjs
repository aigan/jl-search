...
class El extends HTMLElement {
  ...
  constructor() {
    ...
    $el._$label = $el.querySelector(".input-wrapper");
    $el._$feedback = $el.querySelector(".feedback");
  ...
  on_state_click(ev) {
    ...
    // log("Click on state", state, "toggle");
    const opened = $el.hasAttribute("opened");

    if (opened) {
      $el._retain_opened = false;
      return $el.hide_options();
    } else {
  ...
  on_focus(ev) {
    const $el = this;
    const $inp = $el._$inp;
    // log("input focused", $el._retain_opened);
  ...
  get_feedback({ count, speed, query }) {
    ...
    if (count === 1) return `Found one measly results after ${speed / 1000} seconds`;
  ...
  async show_options() {
    ...
    if ($el.hasAttribute("opened")) return;
...
function load_polyfill_popover() {
  if (HTMLElement.prototype.togglePopover) return Promise.resolve();
